...
module "s3_app_log_bucket" {
  ...
  logging = var.app_access_log_bucket_logging_enabled == "yes" ? null : {
...
resource "aws_cloudformation_stack" "trigger_codebuild_stack" {
  ...
  parameters = {
    http_flood_protection_rate_based_rule_activated = local.http_flood_protection_rate_based_rule_activated
    HttpFloodProtectionLogParserActivated           = local.http_flood_protection_log_parser_activated
